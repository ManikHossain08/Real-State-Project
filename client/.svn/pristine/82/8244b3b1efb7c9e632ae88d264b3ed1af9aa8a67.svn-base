...
function initAutocomplete() {
    ...
    $.ajax({
        ...
        success: function (coordinates, type, xhr) {
            ...
            function getSearchedDestinition() {
                alert('getSearchedDestinition');
            ...
            $.ajax({
                ...
                success: function (response, type, xhr) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        success: function (response, type, xhr) {
                            //for (var i = 0; i < response.savedObject.length; i++) {
                            if (response.savedObject.length > 0) getDistanceAndDurationByDriving(response.savedObject[0].placeId, placeId, 'DRIVING');
                            if (response.savedObject.length > 0) getDistanceAndDurationByWalking(response.savedObject[0].placeId, placeId, 'WALKING');
                                //}
        ...
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            $.ajax({
                ...
                success: function (response, type, xhr) {
                    ...
                    if (response.savedObject.length > 0) getDistanceAndDurationByDriving(response.savedObject[0].placeId, response.placeId, response.travelType);
                    else ClearObjectValue();
                },
                error: function (xhr) {
                    window.alert('error: ' + xhr.statusText);
        ...
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            $.ajax({
                ...
                success: function (response, type, xhr) {
                    //ClearObjectValue(response.savedObject, lat, long, placeId);
                    //document.getElementById('message').innerHTML = response;
                    if (response.savedObject.length > 0) getDistanceAndDurationByWalking(response.savedObject[0].placeId, response.placeId, response.travelType);
                    else ClearObjectValue();
                },
                error: function (xhr) {
                    window.alert('error: ' + xhr.statusText);

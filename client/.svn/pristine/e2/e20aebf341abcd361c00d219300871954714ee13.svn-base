...
function initAutocomplete() {
    ...
    var url = 'http://10.1.15.27/GoogleSvc/AdministrationService.svc/getRawMapObject';
    $.ajax({
        type: 'GET',
        url: url,
...
function SaveCoOrdinate(lat, long, objName, Address, objType,id) {
    ...
    $.ajax({
        ...
        success: function (response, type, xhr) {
            ...
            var url = 'http://10.1.15.27/GoogleSvc/AdministrationService.svc/UpdateRawMapObject?objectName=' + objName + '&lat=' + lat + '&longitude=' + long + '&address=' + Address + '&placeId=' + placeId + '&objType=' + objType + '&id=' + id;
            $.ajax({
                type: 'GET',
                url: url,
                dataType: 'json',

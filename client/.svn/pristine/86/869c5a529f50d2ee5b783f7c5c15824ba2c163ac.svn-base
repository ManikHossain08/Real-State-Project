        ...
        if (confirm('Are you want to save this map object?') == true) {
            ...
            $.ajax({
                ...
                success: function (response, type, xhr) {
                    ...
                    var url = 'http://initvent.biz/GoogleAPISvc/AdministrationService.svc/createMapObject?objectName=' + objName + '&lat=' + lat + '&longitude=' + long + '&address=' + Address + '&placeId=' + placeId + '&objType=' + objType;
                    $.ajax({
                        type: 'GET',
                        url: url,
                        dataType: 'json',
...
function insertValue(Source_id, Destinition_id, TravelType, Distance, Time, DistanceInKM) {
    var url = 'http://initvent.biz/GoogleAPISvc/AdministrationService.svc/createComparedMapObject?Source_id=' + Source_id + '&Destinition_id=' + Destinition_id + '&TravelType=' + TravelType + '&Distance=' + Distance + '&Time=' + Time + '&DistanceInKM=' + DistanceInKM;
    $.ajax({
        type: 'GET',
        url: url,
        dataType: 'json',
...
function getDistanceAndDurationByDriving1(destinationId, sourceObjplaceId, travelType) {
    var request = {
        origin: {
    ...
    directionsService.route(request, function (response, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            distanceAndTime.push({
                Source_id: response.request.origin.placeId,
                Destinition_id: response.request.destination.placeId,
                TravelType: response.request.travelMode,
                Distance: response.routes[0].legs[0].distance.value,
                Time: response.routes[0].legs[0].duration.value,
                DistanceInKM: response.routes[0].legs[0].distance.text
            });
            ...
            //        if (response.SalesObjectList.length > 0) getDistanceAndDurationByDriving(response.SalesObjectList[0].placeId, response.placeId, response.travelType);
            //        else ClearObjectValue();
            //    },
            //    error: function (xhr) {
            //        window.alert('error: ' + xhr.statusText);
...
function getDistanceAndDurationByWalking1(destinationId, sourceObjplaceId, travelType) {
    var request = {
        origin: {
            placeId: sourceObjplaceId
        },
    ...
    directionsService.route(request, function (response, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            //        //ClearObjectValue(response.savedObject, lat, long, placeId);
            //        //document.getElementById('message').innerHTML = response;
            //        if (response.SalesObjectList.length > 0) getDistanceAndDurationByWalking(response.SalesObjectList[0].placeId, response.placeId, response.travelType);
            //        else ClearObjectValue();
            //    },
            //    error: function (xhr) {
            //        window.alert('error: ' + xhr.statusText);

...
function initMap() {
    var url = 'http://initvent.biz/GoogleAPISvc/AdministrationService.svc/MapObjects';
    $.ajax({
        type: 'GET',
        url: url,
...
function populateMapObkect(datas) {
    ...
    for (var i = 0; i < datas.length; i++) {
        var optwaypoints = document.createElement('option');
        optwaypoints.innerHTML = datas[i].Title;
        optwaypoints.value = datas[i].placeId;
        waypoints.appendChild(optwaypoints);
    }
    ...
    if (confirm('Are you want to save this object ?' + lat + ',' + long) == true) {
        ...
        $.ajax({
            ...
            success: function (response, type, xhr) {
                ...
                ////////////////////
                var url = 'http://initvent.biz/GoogleAPISvc/AdministrationService.svc/createMapObject?objectName=' + objName + '&lat=' + lat + '&longitude=' + long;
                $.ajax({
                    type: 'GET',
                    url: url,
                    dataType: 'json',
